...
class Interpreter(Memory):
    ...
    def _declare(self, varname, varvalue):
        ...
        if var:
            if re.compile(syntax.func).match(expr):
                ...
                mf = MemoryFrame(callback=self.position, returnvar=var, parameters=func_attrs, function_name=func_groups[0])
                self.stack.push(mf)
    ...
    def define_variable(self, name, vartype):
        if name == "int":
            raise Exception
        var = Variable(name, vartype, copy.deepcopy(self.scope))
        self.mount.push(var)
